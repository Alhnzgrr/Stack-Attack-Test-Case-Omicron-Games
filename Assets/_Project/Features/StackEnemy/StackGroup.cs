...
namespace StackAttack.StackEnemy
{
    public class StackGroup : MonoBehaviour
    {
        ...
        public void Setup(StackGroupEntry entry, float playfieldHalfWidth, float spawnY, float descentSpeed, IScoreSink score, IShardBurst shards, IAudioService audio)
        {
            Cache();
            ...
            for (int i = 0; i < _members.Length; i++)
            {
                ...
                _members[i].Setup(entry.stackType, entry.hp, score, shards, audio);
